...
export default function SummaryReports() {
    ...
    const getAggregatedData = () => {
        ...
        Object.values(departmentMap).forEach(dept => {
            dept.percentageOfServedTokens = dept.totalTokens > 0
                ? ((dept.servedTokens / dept.totalTokens) * 100).toFixed(2)
                : "0.00";
        });

        const grandPercentage = grandTotalTokens > 0
            ? ((grandTotalServedTokens / grandTotalTokens) * 100).toFixed(2)
            : "0.00";

        return { departmentMap, grandTotalTokens, grandTotalSkippedTokens, grandTotalServedTokens, grandPercentage };
    };

    const { departmentMap, grandTotalTokens, grandTotalSkippedTokens, grandTotalServedTokens, grandPercentage } = getAggregatedData();
                    ...
                    <table className="um-table" style={{ width: '100%', textAlign: 'center' }}>
                        ...
                        <tbody>
                            <tr>
                                ...
                                {departmentKeys.map(dept => (
                                    <td key={`total-${dept}`}>{departmentMap[dept].totalTokens}</td>
                                ))}
                            ...
                            <tr>
                                ...
                                {departmentKeys.map(dept => (
                                    <td key={`skip-${dept}`}>{departmentMap[dept].skippedTokens}</td>
                                ))}
                                <td style={{ fontWeight: 'bold' }}>{grandTotalSkippedTokens}</td>
                            </tr>
                            <tr>
                                <td style={{ fontWeight: 'bold' }}>Served Tokens</td>
                                {departmentKeys.map(dept => (
                                    <td key={`served-${dept}`}>{departmentMap[dept].servedTokens}</td>
                                ))}
                            ...
                            <tr>
                                ...
                                {departmentKeys.map(dept => (
                                    <td key={`pct-${dept}`}>{departmentMap[dept].percentageOfServedTokens}%</td>
                                ))}
