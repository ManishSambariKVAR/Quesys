...
export default function SystemSettings() {
  ...
  const handleFactorySubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api.post('/admin/settings/factory', {
        calltoack: callToAck,
        acktoend: ackToEnd,
        endtocall: endToCall,
      });
  ...
  const handleSoftwareSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api.post('/admin/settings/software', {
        recall,
        reassign,
        changeDept,
      });
  ...
  return (
    <div className="um-page">
      ...
      {activeTab === 'factory' && (
        ...
        >
          <h3
            className="um-page-title text-center"
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                <input
                  ...
                  value={callToAck}
                  onChange={(e) => setCallToAck(Number(e.target.value))}
                  required
                />
              </div>
            </div>
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                <input
                  ...
                  className="um-form-control"
                  value={ackToEnd}
                  onChange={(e) => setAckToEnd(Number(e.target.value))}
                  required
                />
              </div>
            </div>
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                <input
                  ...
                  className="um-form-control"
                  value={endToCall}
                  onChange={(e) => setEndToCall(Number(e.target.value))}
                  required
                />
              </div>
            </div>
            <div
              className="um-submit-wrap"
          ...
          <form className="um-form" onSubmit={handleSoftwareSubmit}>
            ...
            >
              <label
                ...
                style={{ margin: 0, flex: 1, textAlign: 'right' }}
              >
                Activate Change Department Button:
              </label>
              <div style={{ flex: 1 }}>
                <select
                  className="um-form-control"
                  value={changeDept}
                  onChange={(e) => setChangeDept(e.target.value)}
                >
                  <option value="true">Enable</option>
                  <option value="false">Disable</option>
                </select>
