...
function calculateTimeDifference(prevInterval, currentInterval) {
  const prevMilliseconds = prevInterval.seconds * 1000 + (prevInterval.milliseconds || 0);
  const currentMilliseconds = currentInterval.seconds * 1000 + (currentInterval.milliseconds || 0);
...
function replaceSpecialForDate(inputString) {
  ...
  return inputString.replace(regex, (match, value) => {
    ...
    switch (value) {
      ...
      case "DD/MM/YY":
        return new Date().toLocaleDateString("en-GB").slice(0, 8).split("/").reverse().join("/");
      case "YYYY/MM/DD":
