...
export default function GrievanceReports() {
  ...
  const getAggregatedData = () => {
    ...
    return {
      ...
      pctSkipped:
        totalTokens > 0
          ? ((skippedTokens / totalTokens) * 100).toFixed(2)
          : '0.00',
      pctServed:
        totalTokens > 0
          ? ((servedTokens / totalTokens) * 100).toFixed(2)
          : '0.00',
      totalTimeFormatted: formatTime(totalTime),
      averageTimeFormatted: formatTime(
        servedTokens > 0 ? totalTime / servedTokens : 0
      ),
    };
  };

  const data = getAggregatedData();
  const grievanceKeys = Object.keys(data.grievancesMap).filter(
    (k) => data.grievancesMap[k] > 0
  );
        ...
        >
          ...
          >
            ...
            <tbody>
              ...
              <tr>
                {grievanceKeys.map((k) => (
                  <td key={k}>{data.grievancesMap[k]}</td>
                ))}
