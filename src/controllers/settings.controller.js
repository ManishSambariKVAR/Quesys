...
async function getFactorySettings(req, res) {
  try {
    ...
    res.json({
      settings: settings || { calltoack: 90, acktoend: 90, endtocall: 90 },
    });
  } catch (error) {
...
async function getSoftwareSettings(req, res) {
  try {
    ...
    res.json({
      settings: settings || {
        activate_recall: 'true',
        activate_reassign: 'true',
        activate_changedept: 'true',
      },
    });
  } catch (error) {
...
async function saveAutoLogout(req, res) {
  ...
  try {
    await settingsService.saveAutoLogoutSettings(
      userId,
      userName,
      userDepartment,
      autoLogoutTime
