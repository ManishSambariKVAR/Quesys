...
export default function OperatorReports() {
    ...
    const getAggregatedData = () => {
        ...
        Object.values(operatorMap).forEach(operator => {
            operator.percentageOfServedTokens = operator.totalTokens > 0
                ? ((operator.servedTokens / operator.totalTokens) * 100).toFixed(2)
                : '0.00';

            operator.pctSkipped = operator.totalTokens > 0
                ? ((operator.skippedTokens / operator.totalTokens) * 100).toFixed(2)
                : '0.00';

            operator.totalTimeFormatted = formatTime(operator.totalTime);

            const avgMs = operator.servedTokens > 0 ? (operator.totalTime / operator.servedTokens) : 0;
            operator.averageTimeFormatted = formatTime(avgMs);

            // Contributions to the grand total
            operator.pctTotalTokens = grandTotalTokens > 0 ? ((operator.totalTokens / grandTotalTokens) * 100).toFixed(2) : '0.00';
            operator.pctSkippedTokens = grandSkippedTokens > 0 ? ((operator.skippedTokens / grandSkippedTokens) * 100).toFixed(2) : '0.00';
            operator.pctServedTokens = grandServedTokens > 0 ? ((operator.servedTokens / grandServedTokens) * 100).toFixed(2) : '0.00';
        });
        ...
        return {
            ...
            grandSkippedTokens,
            grandServedTokens,
            pctGrandSkipped: grandTotalTokens > 0 ? ((grandSkippedTokens / grandTotalTokens) * 100).toFixed(2) : '0.00',
            pctGrandServed: grandTotalTokens > 0 ? ((grandServedTokens / grandTotalTokens) * 100).toFixed(2) : '0.00',
            grandTotalTimeFormatted: formatTime(grandTotalTime),
            grandAverageTimeFormatted: formatTime(grandServedTokens > 0 ? grandTotalTime / grandServedTokens : 0)
        };
    };

    const { operatorMap, grandTotalTokens, grandSkippedTokens, grandServedTokens, pctGrandSkipped, pctGrandServed, grandTotalTimeFormatted, grandAverageTimeFormatted } = getAggregatedData();
        ...
        <div className="user-management">
            ...
            {loading ? (
                ...
                <div className="um-table-wrap" style={{ overflowX: 'auto', whiteSpace: 'nowrap' }}>
                    <table className="um-table um-report-grid" style={{ width: '100%', textAlign: 'center' }}>
                        <thead>
                            <tr>
                                <th></th>
                                {operatorKeys.map(k => (
                                    <th colSpan={2} key={k}>{k}</th>
                                ))}
                                <th>Total</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                ...
                                {operatorKeys.map(k => (
                                    <>
                                        ...
                                        <td key={`total-pct-${k}`}>{operatorMap[k].pctTotalTokens}%</td>
                                    </>
                                ))}
                            ...
                            <tr>
                                ...
                                {operatorKeys.map(k => (
                                    <>
                                        ...
                                        <td key={`skip-pct-${k}`}>{operatorMap[k].pctSkippedTokens}%</td>
                                    </>
                                ))}
                            ...
                            <tr>
                                ...
                                {operatorKeys.map(k => (
                                    <>
                                        ...
                                        <td key={`served-pct-${k}`}>{operatorMap[k].pctServedTokens}%</td>
                                    </>
                                ))}
                            ...
                            <tr>
                                ...
                                {operatorKeys.map(k => (
                                    <>
                                        <td key={`pct-served-${k}`}>{operatorMap[k].percentageOfServedTokens}%</td>
                                        <td>-</td>
                                    </>
                                ))}
                            ...
                            {/* Spacer Row */}
                            <tr>
                                <td colSpan={operatorKeys.length * 2 + 2} style={{ height: '30px', background: 'transparent' }}></td>
                            </tr>
                            <tr>
                                <th style={{ textAlign: 'left' }}>Total Time (mm:ss)</th>
                                {operatorKeys.map(k => (
                                    <>
                                        <td key={`time-${k}`}>{operatorMap[k].totalTimeFormatted}</td>
                                        <td>-</td>
                                    </>
                                ))}
                                <td style={{ fontWeight: 'bold' }}>{grandTotalTimeFormatted}</td>
                            </tr>
                            <tr>
                                <th style={{ textAlign: 'left' }}>Average Time Per Token (mm:ss)</th>
                                {operatorKeys.map(k => (
                                    <>
                                        <td key={`avg-${k}`}>{operatorMap[k].averageTimeFormatted}</td>
                                        <td>-</td>
                                    </>
                                ))}
                                <td style={{ fontWeight: 'bold' }}>{grandAverageTimeFormatted}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
