...
export default function AdminDashboard() {
  ...
  const deptMap: Record<
    string,
    { totalTokens: number; skippedTokens: number; servedTokens: number }
  > = {};
  data.forEach((d) => {
    if (!deptMap[d.dep])
      deptMap[d.dep] = { totalTokens: 0, skippedTokens: 0, servedTokens: 0 };
                ...
                {data.length === 0 ? (
                  ...
                  data.map((dta, i) => (
                    <tr key={i} style={i % 2 === 0 ? s.trEven : s.trOdd}>
                      ...
                      <td style={s.td}>
                        {dta.token_total_count - dta.token_current_count}
                      </td>
                    </tr>
                  ))
                )}
