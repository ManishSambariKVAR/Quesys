...
async function registerDepartment(department, kioskKey, depPrefix, kioskId) {
  const insertQuery = "INSERT INTO departments (department, kiosk_key, dep, kiosk_id) VALUES ($1, $2, $3, $4)";
...
async function updateDepartment(id, department, kioskKey, depPrefix, kioskId) {
  const updateQuery = "UPDATE departments SET department = $1, kiosk_key = $2, dep = $3, kiosk_id = $4 WHERE id = $5";
  await client.query(updateQuery, [department, kioskKey, depPrefix, kioskId, id]);
}
