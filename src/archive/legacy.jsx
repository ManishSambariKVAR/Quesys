...
app.get('/api/dashboard', authenticateToken, async (req, res) => {
  try {
    ...
    // Auto logout
    const autoLogoutResult = await client.query(
      'SELECT * FROM auto_logout_settings'
    );
    const autoLogoutTime = autoLogoutResult.rows[0]?.auto_logout_time || 30;

    // Software settings (feature flags)
    const settingsResult = await client.query(
      'SELECT * FROM software_settings'
    );
...
app.get('/api/admin', authenticateToken, async (req, res) => {
  try {
    ...
    const userLogResult = await client.query(
      'SELECT * FROM userlogs WHERE datetime = $1',
      [currDt]
    );
    const companyResult = await client.query('SELECT * FROM companies');
...
app.post('/api/user/change-department', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await client.query('UPDATE users SET userdept = $1 WHERE id = $2', [
      newDepartment,
      userId,
    ]);
