...
export default function WaitingRoomSettings() {
    ...
    const fetchDisplays = useCallback(async () => {
        try {
            setLoading(true);
            const res = await api.get<{ displays: Display[] }>('/admin/displays/waiting-room');
    ...
    const handleAddSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!displayId || !ipAddress) return showMsg('All fields are required', 'error');

        try {
            await api.post('/admin/displays/waiting-room', { displayId, displayStatus, IP: ipAddress });
            showMsg('Display added successfully', 'success');
            setDisplayId(''); setDisplayStatus('Active'); setIpAddress('');
            fetchDisplays();
    ...
    const handleEditSubmit = async (e: React.FormEvent) => {
        ...
        if (!editItem) return;
        try {
            await api.put(`/admin/displays/waiting-room/${editItem.id}`, { displayId: editItem.display_id, displayStatus: editItem.display_status, IP: editItem.ip_address });
                            ...
                            <tbody>
                                {displays.map((disp, i) => (
                                    <tr key={disp.id}>
                                        ...
                                        <td><div className="um-input-container"><input type="text" value={disp.display_id} readOnly /></div></td>
                                        <td><div className="um-input-container"><input type="text" value={disp.display_status} readOnly /></div></td>
                                        <td><div className="um-input-container"><input type="text" value={disp.ip_address} readOnly /></div></td>
                                        <td>
                                            <button type="button" className="um-edit-button" onClick={() => setEditItem(disp)}>
                                    ...
                                    <tr>
                                        <td>
                                            <div className="um-input-container">
                                                <input type="text" placeholder="Display ID" value={displayId} onChange={e => setDisplayId(e.target.value)} required />
                                            </div>
                                        </td>
                                        <td>
                                            <div className="um-input-container">
                                                <input type="text" placeholder="Status" value={displayStatus} onChange={e => setDisplayStatus(e.target.value)} />
                                            </div>
                                        </td>
                                        <td>
                                            <div className="um-input-container">
                                                <input type="text" placeholder="IP Address" value={ipAddress} onChange={e => setIpAddress(e.target.value)} required />
                                            </div>
                                        </td>
                                        <td>
                            ...
                            <tbody>
                                {displays.map((disp, i) => (
                                    <tr key={disp.id}>
                                        ...
                                        <td><div className="um-input-container"><input type="text" value={disp.ip_address} readOnly /></div></td>
                                        <td>
                                            <button type="button" className="um-delete-button" onClick={() => handleDelete(disp.id)}>
                    ...
                    <div className="um-modal-content">
                        ...
                        <form onSubmit={handleEditSubmit} className="um-modal-form">
                            <div className="um-input-container modal-input" style={{ width: '150px' }}>
                                <input type="text" value={editItem.display_id} onChange={e => setEditItem({ ...editItem, display_id: e.target.value })} placeholder="Display ID" required />
                            </div>
                            <div className="um-input-container modal-input" style={{ width: '150px' }}>
                                <input type="text" value={editItem.display_status} onChange={e => setEditItem({ ...editItem, display_status: e.target.value })} placeholder="Display Status" />
                            </div>
                            <div className="um-input-container modal-input" style={{ width: '150px' }}>
                                <input type="text" value={editItem.ip_address} onChange={e => setEditItem({ ...editItem, ip_address: e.target.value })} placeholder="IP Address" required />
                            </div>
