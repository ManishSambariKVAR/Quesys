...
// GET /api/counters — returns available counters for login dropdown
app.get("/api/counters", async (req, res) => {
  try {
    const currDt = getCurrentDate();
    const updateQuery = `SELECT * FROM userlogs WHERE datetime = $1 AND log = 1;`;
    const data_user_counter = await client.query(updateQuery, [currDt]);

    const update = `SELECT * FROM counterdisplay;`;
...
app.post("/api/login", async (req, res) => {
  try {
    ...
    // Fetch user from DB
    const userRes = await client.query("SELECT * FROM users WHERE userId = $1", [userId]);
    ...
    const depRes = await client.query("SELECT * FROM departments WHERE department = $1", [user.userdept]);
    const kioskData = depRes.rows[0];
...
// GET /api/users — list all users + departments
app.get("/api/users", authenticateToken, async (req, res) => {
  try {
    const usersResult = await client.query("SELECT * FROM users");
    const deptResult = await client.query("SELECT * FROM departments");
    res.json({ users: usersResult.rows, departments: deptResult.rows });
  } catch (error) {
...
// PUT /api/users/update — edit a user
app.put("/api/users/update", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await client.query(updateQuery, [name, userid, userDept, adminLevel, userId]);
...
// GET /api/departments — list all departments + kiosks
app.get("/api/departments", authenticateToken, async (req, res) => {
  try {
    const deptResult = await client.query("SELECT * FROM departments");
    const kioskResult = await client.query("SELECT * FROM kioskRegistration");
    res.json({ departments: deptResult.rows, kiosks: kioskResult.rows });
  } catch (error) {
...
// POST /api/departments/register — create a new department
app.post("/api/departments/register", authenticateToken, async (req, res) => {
  ...
  try {
    await client.query(
      ...
      [department, kioskKey, depPrefix, kioskId]
    );
...
// PUT /api/departments/update — edit a department
app.put("/api/departments/update", authenticateToken, async (req, res) => {
  ...
  try {
    await client.query(
      ...
      [department, kioskKey, depPrefix, kioskId, id]
    );
...
app.get("/api/admin/counters", authenticateToken, async (req, res) => {
  try {
    const counterResult = await client.query("SELECT * FROM counterdisplay");
    const kioskResult = await client.query("SELECT * FROM kioskRegistration");
    res.json({ counters: counterResult.rows, kiosks: kioskResult.rows });
  } catch (error) {
...
app.post("/api/admin/counters/register", authenticateToken, async (req, res) => {
  const { counter, active, displayid, buzzer_time, buzzer_active, blink, ipaddress } = req.body;

  try {
    await client.query(
      `INSERT INTO counterdisplay 
...
app.put("/api/admin/counters/update", authenticateToken, async (req, res) => {
  const { id, counter, active, displayid, buzzer_time, buzzer_active, blink, ipaddress } = req.body;
  ...
  try {
    ...
    await client.query(updateQuery, [
      counter, active, displayid, buzzer_time, buzzer_active, blink, ipaddress, id
    ]);
...
app.delete("/api/admin/counters/:id", authenticateToken, async (req, res) => {
  const id = req.params.id;
  try {
...
app.get("/api/admin/company", authenticateToken, async (req, res) => {
  try {
    const result = await client.query("SELECT * FROM companies ORDER BY id ASC LIMIT 1");
...
app.post("/api/admin/company", authenticateToken, upload.single("logo"), async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      ...
      // Copy instead of rename to avoid cross-device link errors, then delete temp
    ...
    if (result.rows.length > 0) {
      ...
      await client.query(updateQuery, [companyName, result.rows[0].id]);
    } else {
      const insertQuery = "INSERT INTO companies (company_name, logo_path) VALUES ($1, $2)";
      // since we always save as src/uploads/companyLogo.png we just store the generic path
      await client.query(insertQuery, [companyName, "src/uploads/companyLogo.png"]);
    }
...
app.get("/api/admin/ota/files", authenticateToken, async (req, res) => {
  try {
    const dirPath = path.join(__dirname, "src/uploads/OTAForTV/");
...
app.post("/api/admin/ota/files", authenticateToken, upload.none(), async (req, res) => {
...
app.delete("/api/admin/ota/files/:filename", authenticateToken, async (req, res) => {
  try {
    const filePath = path.join(__dirname, "src/uploads/OTAForTV/", req.params.filename);
...
app.get("/api/admin/ota/displays", authenticateToken, async (req, res) => {
  try {
    const waiting = await client.query("SELECT display_id FROM waiting_room_displays");
    const counters = await client.query("SELECT displayid as display_id FROM counterdisplay");
    const assigned = await client.query("SELECT display_id, filename, status FROM otadisplay");

    const allDisplays = [
      ...waiting.rows.map(r => ({ ...r, type: 'Waiting Room' })),
      ...counters.rows.map(r => ({ ...r, type: 'Counter' }))
    ];

    res.json({ displays: allDisplays, linked: assigned.rows });
  } catch (error) {
...
app.post("/api/admin/ota/links", authenticateToken, async (req, res) => {
...
app.get("/api/admin/kiosks", authenticateToken, async (req, res) => {
  try {
...
app.delete("/api/admin/kiosks/:id", authenticateToken, async (req, res) => {
  const { id } = req.params;
  try {
...
app.get("/api/admin/settings/factory", authenticateToken, async (req, res) => {
  try {
    const r = await client.query("SELECT * FROM factory_settings");
    res.json({ settings: r.rows[0] || { calltoack: 90, acktoend: 90, endtocall: 90 } });
  } catch (error) {
    res.status(500).json({ error: "Failed to fetch factory settings." });
  }
});

app.post("/api/admin/settings/factory", authenticateToken, async (req, res) => {
...
app.get("/api/admin/settings/software", authenticateToken, async (req, res) => {
  try {
    const r = await client.query("SELECT * FROM software_settings");
    res.json({ settings: r.rows[0] || { activate_recall: 'true', activate_reassign: 'true', activate_changedept: 'true' } });
  } catch (error) {
    res.status(500).json({ error: "Failed to fetch software settings." });
  }
});

app.post("/api/admin/settings/software", authenticateToken, async (req, res) => {
...
app.get("/api/admin/displays/waiting-room", authenticateToken, async (req, res) => {
  try {
...
app.post("/api/admin/displays/waiting-room", authenticateToken, async (req, res) => {
...
app.put("/api/admin/displays/waiting-room/:id", authenticateToken, async (req, res) => {
  const id = req.params.id;
  const { displayId, displayStatus, IP } = req.body;
  try {
    await client.query(
      "UPDATE waiting_room_displays SET display_id = $1, display_status = $2, ip_address = $3 WHERE id = $4",
      [displayId, displayStatus, IP, id]
    );
...
app.delete("/api/admin/displays/waiting-room/:id", authenticateToken, async (req, res) => {
  const id = req.params.id;
  try {
...
app.get("/api/admin/reports/logs", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Also fetch reassigned data mapping
    const reassignRes = await client.query("SELECT * FROM reassignedTokenData");

    res.json({ logs: result.rows, reassigned: reassignRes.rows });
  } catch (error) {
...
app.get("/api/admin/reports/summary", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // get users & departments for UI 
    const usersRes = await client.query("SELECT name FROM users WHERE adminlevel != 'Admin'");
    const deptsRes = await client.query("SELECT department FROM departments");

    res.json({
      summary: result.rows,
      usernames: usersRes.rows.map(u => u.name),
      departments: deptsRes.rows.map(d => d.department)
    });
  } catch (error) {
...
// GET list of printer report template files
app.get("/api/admin/printer/report-files", authenticateToken, async (req, res) => {
  try {
    const dir = path.join(__dirname, "src", "uploads", "printerReport");
...
// POST save a printer template to disk
app.post("/api/admin/printer/save-template", authenticateToken, async (req, res) => {
  const { content, filename } = req.body;
...
// GET read a specific printer template file content
app.get("/api/admin/printer/template/:filename", authenticateToken, async (req, res) => {
  try {
    const filePath = path.join(__dirname, "src", "uploads", "printerReport", req.params.filename);
...
// POST submit/link a summary report template
app.post("/api/admin/printer/submit-summary", authenticateToken, async (req, res) => {
  const { filename } = req.body;
  if (!filename) return res.status(400).json({ error: "Filename required." });
  try {
    const r = await client.query("SELECT id FROM printer_settings WHERE report_type = 'summary'");
    if (r.rows.length === 0) {
      await client.query("INSERT INTO printer_settings (report_type, uploadlink) VALUES ('summary', $1)", [filename]);
    } else {
      await client.query("UPDATE printer_settings SET uploadlink = $1 WHERE report_type = 'summary'", [filename]);
    }
...
// POST submit/link a token report template
app.post("/api/admin/printer/submit-token", authenticateToken, async (req, res) => {
  const { filename } = req.body;
  if (!filename) return res.status(400).json({ error: "Filename required." });
  try {
    const r = await client.query("SELECT id FROM printer_settings WHERE report_type = 'token'");
    if (r.rows.length === 0) {
      await client.query("INSERT INTO printer_settings (report_type, uploadlink) VALUES ('token', $1)", [filename]);
    } else {
      await client.query("UPDATE printer_settings SET uploadlink = $1 WHERE report_type = 'token'", [filename]);
    }
...
// GET current report linking (which templates are linked)
app.get("/api/admin/printer/view-linking", authenticateToken, async (req, res) => {
  try {
    const summaryRes = await client.query("SELECT uploadlink FROM printer_settings WHERE report_type = 'summary'");
    const tokenRes = await client.query("SELECT uploadlink FROM printer_settings WHERE report_type = 'token'");
    res.json({
      summary: summaryRes.rows[0]?.uploadlink || "Not configured",
      token: tokenRes.rows[0]?.uploadlink || "Not configured"
    });
  } catch (error) {
...
app.post("/api/user/change-department", authenticateToken, async (req, res) => {
  ...
  try {
    await client.query("UPDATE users SET department = $1 WHERE id = $2", [newDepartment, userId]);
