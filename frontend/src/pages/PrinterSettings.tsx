...
export default function PrinterSettings() {
    ...
    const handleSaveTemplate = async () => {
        ...
        try {
            setLoading(true);
            await api.post('/admin/printer/save-template', { content: templateContent, filename });
    ...
    const handleLoadTemplate = async (filename: string) => {
        try {
            const res = await api.get(`/admin/printer/template/${encodeURIComponent(filename)}`);
    ...
    const handleSubmitSummary = async () => {
        ...
        try {
            setLoading(true);
            await api.post('/admin/printer/submit-summary', { filename: selectedSummaryFile });
    ...
    const handleSubmitToken = async () => {
        ...
        try {
            setLoading(true);
            await api.post('/admin/printer/submit-token', { filename: selectedTokenFile });
    ...
    const handleViewFile = (filename: string) => {
        ...
        window.open(`/src/uploads/printerReport/${encodeURIComponent(filename)}`, '_blank');
    };
                ...
                <div className="um-content">
                    ...
                    {fileList.length > 0 && (
                        <div style={{ marginBottom: 16, display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                            <label className="um-label" style={{ marginBottom: 0 }}>Load existing:</label>
                            <select
                    ...
                    <textarea
                        ...
                        style={{
                            width: '100%', minHeight: 300, padding: 12, fontFamily: 'monospace',
                            fontSize: 14, border: '1px solid #ddd', borderRadius: 8, resize: 'vertical',
                        }}
                    />
                    <div style={{ display: 'flex', gap: 12, marginTop: 16 }}>
                        <button className="um-submit-btn" style={{ width: 'auto', padding: '10px 20px', backgroundColor: '#6c757d' }} onClick={handleDownloadTemplate}>
                            ⬇️ Download Template
                        </button>
                        <button className="um-submit-btn" style={{ width: 'auto', padding: '10px 20px' }} onClick={handleSaveTemplate} disabled={loading}>
                ...
                <div className="um-content" style={{ maxWidth: 800 }}>
                    ...
                    <div className="um-table-wrapper">
                        <table className="um-table">
                            ...
                            <tbody>
                                <tr>
                                    <td><div className="um-input-container"><input type="text" value="Summary Report" readOnly /></div></td>
                                    <td>
                                        <div className="um-input-container">
                                            <select className="um-form-control" value={selectedSummaryFile} onChange={e => setSelectedSummaryFile(e.target.value)} style={{ padding: '8px', border: 'none' }}>
                                    ...
                                    <td>
                                        <button className="um-edit-button" onClick={() => handleViewFile(selectedSummaryFile)} style={{ color: '#666' }}>
                                            <i className="fa fa-eye"></i>
                                        </button>
                                    </td>
                                    <td>
                                        <button className="um-edit-button" onClick={handleSubmitSummary} disabled={loading}>
                ...
                <div className="um-content" style={{ maxWidth: 800 }}>
                    ...
                    <div className="um-table-wrapper">
                        <table className="um-table">
                            ...
                            <tbody>
                                <tr>
                                    <td><div className="um-input-container"><input type="text" value="Token Report" readOnly /></div></td>
                                    <td>
                                        <div className="um-input-container">
                                            <select className="um-form-control" value={selectedTokenFile} onChange={e => setSelectedTokenFile(e.target.value)} style={{ padding: '8px', border: 'none' }}>
                                                <option value="" disabled>Select template</option>
                                                {fileList.length > 0
                                                    ? fileList.map(f => <option key={f} value={f}>{f}</option>)
                                                    : <option disabled>No reports available.</option>
                                    ...
                                    <td>
                                        <button className="um-edit-button" onClick={() => handleViewFile(selectedTokenFile)} style={{ color: '#666' }}>
                                            <i className="fa fa-eye"></i>
                                        </button>
                                    </td>
                                    <td>
                                        <button className="um-edit-button" onClick={handleSubmitToken} disabled={loading}>
                                            <i className="fa fa-save"></i>
                                        </button>
                                    </td>
                                </tr>
                ...
                <div className="um-content" style={{ maxWidth: 800 }}>
                    ...
                    <div className="um-table-wrapper">
                        <table className="um-table">
                            ...
                            <tbody>
                                <tr>
                                    <td><div className="um-input-container"><input type="text" value="Selected Template" readOnly style={{ textAlign: 'left', paddingLeft: '15px' }} /></div></td>
                                    <td><div className="um-input-container"><input type="text" value={summaryLink} readOnly /></div></td>
                                    <td><div className="um-input-container"><input type="text" value={tokenLink} readOnly /></div></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
